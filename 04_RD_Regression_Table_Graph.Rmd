...
```{r read}
rd <- read_excel("meta_regression_table.xlsx", sheet="Sheet2") %>% 
  clean_names() %>% 
  drop_na(group) %>% 
...
rd %>% 
  ...
  annotate(
    ...
    x = 1, y=0.5, size = 7, color = "DarkGrey", hjust="left") +
  annotate(
...
ggsave("rd_determinants_net_survival.png")
```
